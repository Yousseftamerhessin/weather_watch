...
import 'package:weather_app/cubit/fetchWeatherData.dart' as fetch_weather;

class WeatherCard extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<fetch_weather.HomeCubit, fetch_weather.HomeState>(
      builder: (context, state) {
        if (state is fetch_weather.HomeLoading) {
          return Center(child: CircularProgressIndicator());
        }

        if (state is fetch_weather.HomeError) {
          return Center(child: Text('Error: ${state.message}'));
        }

        if (state is fetch_weather.HomeLoaded) {
          final data = state.currentWeatherData;
